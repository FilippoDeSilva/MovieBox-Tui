...
mod tests {
    ...
    #[test]
    fn test_deserialize_cinemeta_detail() {
        let json_str = r#"{
            "meta": {
                "id": "tt1375666",
                "name": "Inception",
        ...
        let meta = val.get("meta").unwrap().clone();
        let detail: MetaDetail =
            serde_json::from_value(meta).expect("Failed to deserialize MetaDetail");
        assert_eq!(detail.id, "tt1375666");
        assert_eq!(detail.name, "Inception");
        assert_eq!(detail.imdb_rating.as_deref(), Some("8.8"));
        assert_eq!(detail.director, vec!["Christopher Nolan"]);
        assert_eq!(
            detail.cast,
