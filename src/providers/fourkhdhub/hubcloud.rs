...
mod tests {
    ...
    #[test]
    fn rejects_local_and_non_media_destinations() {
        assert!(validate_playback_url("https://127.0.0.1/video.mp4").is_err());
        assert!(validate_playback_url("https://example.com/login.php").is_err());
        assert!(validate_playback_url("https://example.com/pack.zip").is_err());
        assert!(validate_playback_url("https://cdn.example.com/video.mp4").is_ok());
    }

    #[test]
    fn pixel_gateway_precedes_fsl() {
        assert!(
            score("https://pixel.hubcloud.cx/?id=x", "Download")
                < score("https://hub.latent.click/file", "FSL")
        );
    }

    #[test]
    fn extracts_hidden_pixeldrain_api_source() {
        let html = r#"
          <a href="https://pixeldrain.dev/u/dead">Pixel Server</a>
          <script>var pxl = "https://pixeldrain.dev/u/8UJ3KFve";</script>
        "#;
        assert_eq!(
            extract_script_pixeldrain_urls(html),
            vec!["https://pixeldrain.dev/api/file/8UJ3KFve?download"]
        );
    }

    #[test]
    fn extracts_hubcloud_fallback_from_hubdrive() {
        let html = r#"
          <a href="/login">Login</a>
          <a href="https://hubcloud.cx/drive/pss5r5pacmsn1lp">HubCloud Server</a>
        "#;
        assert_eq!(
            extract_hubcloud_drive_url(html).as_deref(),
            Some("https://hubcloud.cx/drive/pss5r5pacmsn1lp")
        );
    }
}
