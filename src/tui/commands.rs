...
mod tests {
    ...
    fn test_mode_availability() {
        ...
        let streaming_commands = SlashCommand::suggest(&state, "/");
        assert!(streaming_commands.contains(&SlashCommand::Browse));
        assert!(streaming_commands.contains(&SlashCommand::History));
        assert!(!streaming_commands.contains(&SlashCommand::List));
        assert!(!streaming_commands.contains(&SlashCommand::Config));

        state.is_tv_mode = true;
        let tv_commands = SlashCommand::suggest(&state, "/");
        assert!(tv_commands.contains(&SlashCommand::List));
        assert!(tv_commands.contains(&SlashCommand::Config));
        assert!(tv_commands.contains(&SlashCommand::Reload));
        assert!(!tv_commands.contains(&SlashCommand::Browse));
        assert!(!tv_commands.contains(&SlashCommand::History));

        state.is_tv_mode = false;
        state.is_addon_mode = true;
        state.addons_enabled = true;
        let addon_commands = SlashCommand::suggest(&state, "/");
        assert!(addon_commands.contains(&SlashCommand::Addons));
        assert!(!addon_commands.contains(&SlashCommand::Config));
        assert!(!addon_commands.contains(&SlashCommand::Browse));
        assert!(!addon_commands.contains(&SlashCommand::List));
    }
    ...
    fn test_parse_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/download-dir ~/Movies"),
            Some(ParsedCommand::DownloadDir("~/Movies"))
        );
        assert_eq!(
            SlashCommand::parse("/download-dir"),
            Some(ParsedCommand::DownloadDir(""))
        );
