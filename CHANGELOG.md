...
### Added
- **Unified Interactive Settings & Preferences Hub (`/settings`)**: Introduced a consolidated, mouse-friendly modal dialog replacing fragmented slash commands. Features 4 categorized tabs (`General`, `Content Modes`, `Appearance`, `Maintenance`), live player cycling (`Auto Detect`, `mpv`, `VLC`, `IINA`, `Android`), live theme cycling with palette preview swatches, inline download directory editing, and multi-mode toggling with safety guards. Streamlined slash command autocomplete suggestions while retaining full legacy command compatibility.
