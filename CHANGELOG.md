...
- **Streamlined Playback & "Open With" Removal (`src/tui/`)**: Removed the redundant `[o] Open With` action and shortcuts from the Details footer, keyboard handlers, mouse hitboxes, and help dialogs in favor of configuring default media players (`mpv`, `VLC`, `IINA`, `Android`) directly in the Settings Hub (`/settings`).
