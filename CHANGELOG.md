...
### Added
- **Multi-Resolution DASH stream recognition & [Multi] badge**: Identifies adaptive multi-bitrate streams and MPEG-DASH manifests (`index.mpd`), labeling them with a distinct `[Multi]` badge in the streams table while stripping confusing raw CDN resolution suffixes (like `_360P`) from release titles.
- **Seekable 4KHDHub stream resolution**: Prioritized high-speed seekable Cloudflare R2 / S3 / FSL streams (`score = 0`) supporting `HTTP 206 Partial Content` with `Accept-Ranges: bytes` over non-seekable Google UserContent attachment downloads.
- **Broadened download button selectors**: Recognizes HubCloud variations (`a#download`, `a.btn-primary`, `a.btn-success`, `a[href*='/download/']`, `a[href*='gamerxyt.com']`).
- **Responsive details header layout**: Content-adaptive height based on metadata density, 2:3 aspect ratio poster thumbnail, dedicated bold title row, separate badges row, breathing spacer, and audio dubs fallback.
- **Seamless in-place provider switching**: Pressing `Ctrl+P` on the details screen re-queries the new provider for the current title without kicking the user back to the home landing.
- **Ultra-minimal no-results screen**: Borderless search bar anchored at Row 0 with `0 results` indicator, single headline naming query and provider, and smart action buttons (`[ Try on <Provider> (^P) ]`, `[ Clear Search (c) ]`).
- **1:1 mouse hitboxes**: Clickable actions on no-results screen and streams panel.
- **Settled-state lifecycle tracking**: `has_streams_settled` and `has_search_settled` flags prevent transient "No stream sources found" or "No matches found" flashes during async transitions.
- **Terminal truecolor detection hardening**: Added native classification for `foot` and `alacritty` even when `COLORTERM` is stripped.
- **Unified Termux environment detection**: Harmonized `is_termux_env` across player and updater modules.
- **MovieBox visitor login & session persistence**: Implemented visitor authentication with atomic disk caching (`moviebox_session.bin`) and MPEG-DASH manifest decoding.
- **First-run discover shelf**: Added curated discover shelf on the home landing screen when favorites are empty.

### Changed
- **Streams table header**: Replaced unavailable `DURATION` column (`--:--`) with `SOURCE` label, expanding `MEDIA TAGS` and `RELEASE` widths.
...
### Performance
- **MessagePack binary caching**: Replaced JSON file cache with `rmp-serde` binary serialization and `MBC1` magic header envelopes, delivering faster disk reads and smaller cache files.
- **Zero-copy IPTV M3U parser**: Replaced format-based attribute extraction with zero-copy slice scanning.
- **Pre-compiled CSS selectors**: Replaced runtime selector parsing with static `LazyLock<Selector>` instances.
- **Global static DNS resolver**: Unified DNS resolution via `hickory-resolver` with embedded Cloudflare/Google/Quad9 public fallbacks.
