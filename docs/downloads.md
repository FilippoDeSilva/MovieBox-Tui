...
- **Custom path:** Users can set a custom download directory using `/download-dir <path>` or revert with `/download-dir reset`. Target directories are validated with active write probes before saving, consistently creating the organized `MovieBox-TUI` subfolder hierarchy (`Movies/` and `Series/`), and the engine automatically falls back to default if custom storage becomes unavailable at runtime.
...
- **Duplication prevention**: When starting a download or processing a season batch queue, the engine checks if the target media file is already completed on disk. Existing completed episodes are automatically skipped to avoid duplicate downloads and bandwidth consumption.
...
Selected subtitle sidecars are saved next to the video using standard ISO 639-1 language codes (e.g. `.en.srt`, `.hi.srt`) and supported subtitle extensions (`.srt`, `.vtt`, `.ass`, `.ssa`, `.sub`) so media players and media servers identify the track language automatically.
