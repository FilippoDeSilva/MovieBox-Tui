...
- Automated testing is enforced via `cargo test --all-features --locked` covering 207 unit and integration tests across 16 test suites (see [`docs/testing.md`](testing.md)). The count is updated when tests change; it does not replace real-player and real-device verification.
