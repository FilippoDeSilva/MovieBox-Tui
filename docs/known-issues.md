...
- Automated testing is enforced via `cargo test --all-features --locked` covering 296+ unit and integration tests across 19 test suites (see [`docs/testing.md`](testing.md)). The count is updated when tests change; it does not replace real-player and real-device verification.
